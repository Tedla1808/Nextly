document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateCalendar = (date) => { if (!calendarDaysGrid || !monthYearHeader) return; calendarDaysGrid.innerHTML = ''; const today = new Date(); today.setHours(0, 0, 0, 0); const month = date.getMonth(), year = date.getFullYear(); monthYearHeader.textContent = `${date.toLocaleString('default', { month: 'long' })} ${year}`; const firstDayOfMonth = new Date(year, month, 1); const daysInMonth = new Date(year, month + 1, 0).getDate(); const startingDayOfWeek = firstDayOfMonth.getDay(); for (let i = 0; i < startingDayOfWeek; i++) calendarDaysGrid.insertAdjacentHTML('beforeend', `<div class="calendar-day not-current-month"></div>`); for (let i = 1; i <= daysInMonth; i++) { const dayCell = document.createElement('div'); dayCell.className = 'calendar-day'; dayCell.textContent = i; const cellDate = new Date(year, month, i); const formattedCellDate = formatDate(cellDate); dayCell.dataset.date = formattedCellDate; if (today.getTime() === cellDate.getTime()) dayCell.classList.add('current-day'); if (tasks[formattedCellDate]?.some(t => !t.completed)) dayCell.classList.add('day-with-task'); if (formatDate(currentDate) === formattedCellDate) dayCell.classList.add('selected-day'); calendarDaysGrid.appendChild(dayCell); } };
    const updateUIForNewDate = () => { currentDate.setHours(0, 0, 0, 0); if (datePicker) datePicker.value = formatDate(currentDate); if (selectedDateDisplay) selectedDateDisplay.textContent = currentDate.toLocaleDateString('default', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }); if (searchBar) searchBar.value = ''; if (selectedDateDisplay) selectedDateDisplay.classList.remove('hidden'); const calContainer = document.querySelector('.calendar-container'); if (calContainer) calContainer.classList.remove('hidden'); generateCalendar(currentDate); renderTasks(); };
    ...
    const openJumpToDateModal = () => { if (!jumpMonthSelect || !jumpDaySelect || !jumpYearInput || !jumpToDateModal) return; if (jumpMonthSelect.children.length === 0) { const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]; months.forEach((month, index) => jumpMonthSelect.insertAdjacentHTML('beforeend', `<option value="${index}">${month}</option>`)); } if (jumpDaySelect.children.length === 0) { for (let i = 1; i <= 31; i++) jumpDaySelect.insertAdjacentHTML('beforeend', `<option value="${i}">${i}</option>`); } jumpMonthSelect.value = currentDate.getMonth(); jumpDaySelect.value = currentDate.getDate(); jumpYearInput.value = currentDate.getFullYear(); jumpToDateModal.classList.remove('hidden'); };
    ...
    if (calendarDaysGrid) calendarDaysGrid.addEventListener('click', (e) => { const target = e.target.closest('.calendar-day'); if (target && !target.classList.contains('not-current-month')) { currentDate.setDate(parseInt(target.textContent, 10)); updateUIForNewDate(); } });
    ...
    if (goToDateBtn) goToDateBtn.addEventListener('click', () => { if (!jumpYearInput || !jumpMonthSelect || !jumpDaySelect) return; const year = parseInt(jumpYearInput.value, 10); if (!isNaN(year) && year > 1000 && year < 9999) { currentDate = new Date(year, parseInt(jumpMonthSelect.value, 10), parseInt(jumpDaySelect.value, 10)); updateUIForNewDate(); closeJumpToDateModal(); } else { alert("Please enter a valid year."); } });
