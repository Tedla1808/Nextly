document.addEventListener('DOMContentLoaded', () => {
    ...
    const logout = () => { currentUser = null; localStorage.removeItem('nextlyUser'); localStorage.removeItem('nextlySettings_' + currentUser); tasks = {}; body.classList.remove('logged-in'); if (currentUserDisplay) currentUserDisplay.textContent = ''; if (loginView) loginView.classList.remove('hidden'); if (profileView) profileView.classList.add('hidden'); if (!isLoginMode) toggleAuthMode(); loadAllUserData(); };
    ...
    const openMenu = () => body.classList.add('side-menu-active');
    const closeMenu = () => { body.classList.remove('side-menu-active'); if (searchBar && searchBar.value !== '') { searchBar.value = ''; updateUIForNewDate(); } };
    const openTaskModal = (task = null) => { if (!taskForm || !modalTitle || !editTaskDateContainer || !taskModal) return; taskForm.reset(); if (task) { modalTitle.textContent = 'Edit Task'; currentTaskId = task.id; originalTaskDate = task.date; getEl('task-id').value = task.id; getEl('task-title').value = task.title; getEl('task-description').value = task.description; getEl('task-priority').value = task.priority; if (editTaskDateInput) editTaskDateInput.value = originalTaskDate; editTaskDateContainer.classList.remove('hidden'); } else { modalTitle.textContent = 'Add New Task'; currentTaskId = null; originalTaskDate = null; editTaskDateContainer.classList.add('hidden'); } taskModal.classList.remove('hidden'); };
    ...
    const openJumpToDateModal = () => { if (!jumpMonthSelect || !jumpDaySelect || !jumpYearInput || !jumpToDateModal) return; const monthSource = calendarType === 'gregorian' ? ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"] : amharicMonthNames; jumpMonthSelect.innerHTML = ''; monthSource.forEach((month, index) => jumpMonthSelect.insertAdjacentHTML('beforeend', `<option value="${index}">${month}</option>`)); if (jumpDaySelect.children.length === 0) { for (let i = 1; i <= 31; i++) jumpDaySelect.insertAdjacentHTML('beforeend', `<option value="${i}">${i}</option>`); } const [year, month, day] = calendarType === 'gregorian' ? [currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()] : EthiopianDate.toEthiopian(currentDate.getFullYear(), currentDate.getMonth() + 1, currentDate.getDate()); jumpMonthSelect.value = month - (calendarType === 'ethiopian' ? 1 : 0); jumpDaySelect.value = day; jumpYearInput.value = year; jumpToDateModal.classList.remove('hidden'); };
    const closeJumpToDateModal = () => { if (jumpToDateModal) jumpToDateModal.classList.add('hidden'); };
    function addSwipeAndDragListeners(element) { let isDragging = false, startX, currentX, diff = 0; const getClientX = (e) => e.touches ? e.touches[0].clientX : e.clientX; const dragStart = (e) => { startX = getClientX(e); isDragging = true; element.style.transition = ''; }; const dragMove = (e) => { if (!isDragging || element.classList.contains('dragging')) return; currentX = getClientX(e); diff = currentX - startX; if (Math.abs(diff) > 10) element.style.transform = `translateX(${diff}px)`; }; const dragEnd = () => { if (!isDragging) return; isDragging = false; element.style.transition = 'transform 0.3s ease'; if (diff > 100) showCompleteConfirmModal(element.dataset.id); else if (diff < -100) showDeleteConfirmModal(element.dataset.id); element.style.transform = 'translateX(0)'; diff = 0; }; element.addEventListener('touchstart', dragStart); element.addEventListener('touchmove', dragMove); element.addEventListener('touchend', dragEnd); element.addEventListener('mousedown', dragStart); element.addEventListener('mousemove', dragMove); element.addEventListener('mouseup', dragEnd); element.addEventListener('mouseleave', () => { if (isDragging) dragEnd(); }); }
    const calculateAndDisplayStats = () => { if (!completedCountEl || !overdueCountEl || !totalCountEl || !completionRateEl) return; let totalTasks = 0, completedTasks = 0, overdueTasks = 0; const today = new Date(); today.setHours(0, 0, 0, 0); Object.keys(tasks).forEach(dateKey => { tasks[dateKey].forEach(task => { totalTasks++; const taskDate = new Date(dateKey + 'T12:00:00'); if (task.completed) { completedTasks++; } else if (taskDate < today) { overdueTasks++; } }); }); const completionRate = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0; completedCountEl.textContent = completedTasks; overdueCountEl.textContent = overdueTasks; totalCountEl.textContent = totalTasks; completionRateEl.textContent = `${completionRate}%`; const last7DaysLabels = [], last7DaysData = []; for (let i = 6; i >= 0; i--) { const date = new Date(); date.setDate(date.getDate() - i); const formattedDate = formatDate(date); last7DaysLabels.push(date.toLocaleDateString('default', { weekday: 'short' })); last7DaysData.push((tasks[formattedDate] || []).filter(task => task.completed).length); } renderProductivityChart(last7DaysLabels, last7DaysData); };
    const renderProductivityChart = (labels, data) => { if (!productivityChartCanvas) return; if (productivityChart) productivityChart.destroy(); const accentColor = getComputedStyle(document.body).getPropertyValue('--primary-color'); productivityChart = new Chart(productivityChartCanvas, { type: 'bar', data: { labels, datasets: [{ label: 'Tasks Completed', data, backgroundColor: accentColor + '80', borderColor: accentColor, borderWidth: 1, borderRadius: 5 }] }, options: { responsive: true, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } }, plugins: { legend: { display: false } } } }); };
    ...
    if (searchBar) searchBar.addEventListener('input', (e) => { const query = e.target.value.toLowerCase().trim(); if (query === '') { updateUIForNewDate(); return; } if (selectedDateDisplay) selectedDateDisplay.classList.add('hidden'); const calContainer = document.querySelector('.calendar-container'); if (calContainer) calContainer.classList.add('hidden'); const allTasks = []; for (const dateKey in tasks) { tasks[dateKey].forEach(task => allTasks.push({ ...task, date: dateKey })); } const results = allTasks.filter(task => task.title.toLowerCase().includes(query) || task.description.toLowerCase().includes(query)); renderSearchResults(results, query); });
    if (themeToggle) themeToggle.addEventListener('click', () => { settings.theme = themeToggle.checked ? 'dark' : 'light'; applyTheme(settings.theme); if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); if (themeSelector) { themeSelector.querySelector('.active')?.classList.remove('active'); const activeBtn = themeSelector.querySelector(`button[value="${settings.theme}"]`); if (activeBtn) activeBtn.classList.add('active'); } });
    ...
    if (goToDateBtn) goToDateBtn.addEventListener('click', () => { if (!jumpYearInput || !jumpMonthSelect || !jumpDaySelect) return; const year = parseInt(jumpYearInput.value, 10); if (isNaN(year) || year < 1000 || year > 9999) { alert("Please enter a valid year."); return; } const month = parseInt(jumpMonthSelect.value, 10); const day = parseInt(jumpDaySelect.value, 10); if (calendarType === 'gregorian') { currentDate = new Date(year, month, day); } else { const gregorian = EthiopianDate.toGregorian(year, month + 1, day); currentDate = new Date(gregorian.year, gregorian.month - 1, gregorian.day); } updateUIForNewDate(); closeJumpToDateModal(); });
    ...
    if (taskForm) taskForm.addEventListener('submit', async (e) => { e.preventDefault(); if (!currentUser) return alert('Please log in to save tasks.'); const taskData = { title: getEl('task-title').value, description: getEl('task-description').value, priority: getEl('task-priority').value }; if (currentTaskId) { const newDate = getEl('edit-task-date').value; await fetch(`/api/tasks/${currentUser}/${currentTaskId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...taskData, date: newDate }) }); } else { const selectedDate = getEl('date-picker').value; await fetch(`/api/tasks/${currentUser}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...taskData, date: selectedDate }) }); } await loadTasksForCurrentUser(); closeTaskModal(); updateUIForNewDate(); });
    if (confirmDeleteBtn) confirmDeleteBtn.addEventListener('click', async () => { if (!currentUser || !taskIdToDelete) return; await fetch(`/api/tasks/${currentUser}/${taskIdToDelete}`, { method: 'DELETE' }); await loadTasksForCurrentUser(); hideDeleteConfirmModal(); updateUIForNewDate(); });
    if (confirmCompleteBtn) confirmCompleteBtn.addEventListener('click', async () => { if (!currentUser || !taskIdToComplete) return; await fetch(`/api/tasks/${currentUser}/${taskIdToComplete}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ completed: true }) }); await loadTasksForCurrentUser(); hideCompleteConfirmModal(); updateUIForNewDate(); });
    if (closeSettingsBtn) closeSettingsBtn.addEventListener('click', closeSettingsModal);
    if (closeAnalysisBtn) closeAnalysisBtn.addEventListener('click', closeAnalysisModal);
    if (enableNotificationsToggle) enableNotificationsToggle.addEventListener('change', (e) => { settings.notificationsEnabled = e.target.checked; if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); });
    if (dailySummaryToggle) dailySummaryToggle.addEventListener('change', (e) => { settings.dailySummaryEnabled = e.target.checked; if (dailySummaryTimeContainer) dailySummaryTimeContainer.classList.toggle('hidden', !e.target.checked); if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); });
    if (dailySummaryTimeInput) dailySummaryTimeInput.addEventListener('change', (e) => { settings.dailySummaryTime = e.target.value; if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); });
    if (hourlyReminderToggle) hourlyReminderToggle.addEventListener('change', (e) => { settings.hourlyReminderEnabled = e.target.checked; if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); });
    if (themeSelector) themeSelector.addEventListener('click', (e) => { const button = e.target.closest('button'); if (button) { const activeBtn = themeSelector.querySelector('.active'); if (activeBtn) activeBtn.classList.remove('active'); button.classList.add('active'); settings.theme = button.value; applyTheme(settings.theme); if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); } });
    if (accentColorSelector) accentColorSelector.addEventListener('click', (e) => { const swatch = e.target.closest('.color-swatch'); if (swatch) { const activeSwatch = accentColorSelector.querySelector('.active'); if (activeSwatch) activeSwatch.classList.remove('active'); swatch.classList.add('active'); settings.accentColor = swatch.dataset.color; applyAccentColor(settings.accentColor); if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); } });
    if (fontSizeSelector) fontSizeSelector.addEventListener('click', (e) => { const button = e.target.closest('button'); if (button) { const activeBtn = fontSizeSelector.querySelector('.active'); if (activeBtn) activeBtn.classList.remove('active'); button.classList.add('active'); settings.fontSize = button.value; applyFontSize(settings.fontSize); if (currentUser) localStorage.setItem(`nextlySettings_${currentUser}`, JSON.stringify(settings)); saveSettingsToServer(); } });
